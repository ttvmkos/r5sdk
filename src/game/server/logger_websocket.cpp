...
namespace LOGGER
{
    ...
    bool WebSocketCommandHandler::Connect(const char* address, int port)
    {
        ...
        if (!m_initialized.load())
        {
            ...
            params.bufSize = 262144; //was 4096, now 256kb
            params.retryTime = 2.0f;
            params.maxRetries = 5;
            params.timeOut = 60;
            params.keepAlive = 30;
            params.laxSSL = 0;

            std::string addressStr = CFmtStr("ws://%s:%d", address, port).Get();
    ...
    void WebSocketCommandHandler::RunFrame()
    {
        ...
        if (currentTime - lastWebSocketUpdate >= 0.10) // 100ms throttle
        {
            getInstance().Update();
